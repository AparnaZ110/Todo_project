...
# HTTP POST request (username,email,password 1,2)   
    if request.method == "POST":
        ...
        messages.success(request, "Registration sucessful. Please log in.")
...
def admin_page(request):
    ...
    if search_query:
        user = User.objects.filter(username__istartswith = search_query).exclude(
        is_staff = True
    )
    else:
        users = User.objects.all().exclude(is_staff = True)
    response = render(request, "admin_page.html",{"users":users, "search_query": search_query})
    return response
